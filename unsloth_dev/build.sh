...
usage() {
    ...
    echo "  -m, --models-dir     Directory containing models to be mounted (default: $MODELS_DIR)"
    echo "  -D, --datasets-dir   Directory containing datasets to be mounted (default: $DATASETS_DIR)"
...
clone_repos_dev() {
    ...
    if [ ! -d "$BASE_DIR/$UNSLOTH_DIR" ]; then
        echo "Cloning into $UNSLOTH_REPO_URL..."
        git clone $UNSLOTH_REPO_URL -b $UNSLOTH_REPO_BRANCH $UNSLOTH_DIR
...
if [ -z "$MODELS_DIR" ] || [ ! -d "$MODELS_DIR" ] ||
   [ -z "$(ls -A "$MODELS_DIR" 2>/dev/null)" ]; then
    echo "Error: MODELS_DIR is not set or does not exist or directory is empty."
    echo "MODELS_DIR=$MODELS_DIR"
    echo "Please set the MODELS_DIR variable to a valid directory containing models."
    exit 1
fi

# Check if DATASETS_DIR is set and exists
if [ -z "$DATASETS_DIR" ] || [ ! -d "$DATASETS_DIR" ] ||
   [ -z "$(ls -A "$DATASETS_DIR" 2>/dev/null)" ]; then
    echo "Error: DATASETS_DIR is not set or does not exist or the directory is empty."
    echo "DATASETS_DIR=$DATASETS_DIR"
    echo "Please set the DATASETS_DIR variable to a valid directory containing datasets."
...
# Build the Docker image
eval "docker build --no-cache $BUILD_ARGS -t \"$IMAGE_NAME:$IMAGE_TAG\" ."
...
if [ "$(docker ps -q -f name=${CONTAINER_NAME})" ]; then
    ...
    if [ "$DEV_MODE" = true ]; then
        ...
        BUILD_CMD=" \
            ...
            cd ${UNSLOTH_DOCKER_DIR} && python setup.py bdist_wheel; \
            pip install ${UNSLOTH_DOCKER_DIR}/dist/*.whl; \
        "
